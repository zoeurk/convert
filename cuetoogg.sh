	...
	then
		...
		do
			...
			then
				...
				ARTIST=`head -n 2 /tmp/songs.txt | tail -n 1 | sed 's/perfomer:[\t ]*//'`
				...
				encode 0 $LEN "${CUEFILE%*.cue}.flac" Music_/"$ARTIST - $YEARS - $ALBUM - $TRACK - $TITLE.ogg"
			else
				...
				mv /tmp/bc.script1.bak /tmp/bc.script1
				mv /tmp/bc.script2.bak /tmp/bc.script2
				encode 1 $DURATION "$START" "${CUEFILE%*.cue}.flac" "Music_/$ARTIST - $YEARS - $ALBUM - $TRACK - $TITLE.ogg"
			fi
		done
		cat /tmp/songs.txt
		YEARS=`echo $CUEFILE | sed 's@^.*/\([0-9]*\( - [0-9]*\)\?\) .*$@\1@'`
		...
		mv /tmp/bc.script1.bak /tmp/bc.script1
		mv /tmp/bc.script2.bak /tmp/bc.script2
		encode 2 $START "${CUEFILE%*.cue}.flac" "Music_/$ARTIST - $YEARS - $ALBUM - $TRACK - $TITLE.ogg"
	fi
done < /tmp/list.cue
